...
const User = require("../../models/Bartender");

router.get("/", (req, res) => {
  UserReview.find({})
    .sort({ date: -1 })
    .then((bartenderReview) => res.json(bartenderReview))
    .catch((err) => res.status(404).json({ noBartenderReviewFound: "No reviews found" }));
});

router.get("/bartenderReview/:user_id", (req, res) => {
...
router.post(
  ...
  (req, res) => {
    ...
    bartenderReview.findById(req.params.id) //id?
      ...
      .then(() => {
        if (theBartenderReview.length === 0) {
          ...
          newBartenderReview.save().then((bartenderReview) => res.json(bartenderReview));
        } else {
          return res
            .status(400)
